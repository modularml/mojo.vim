" Support for Mojo indenting, see runtime/indent/mojo.vim

let s:keepcpo= &cpo
set cpo&vim

" need to inspect some old g:pyindent_* variables to be backward compatible
let g:mojo_indent = extend(get(g:, 'mojo_indent', {}), #{
  ...
  "\ g:mojo_indent.searchpair_timeout
...
function s:SearchBracket(fromlnum, flags)
  return searchpairpos('[[({]', '', '[])}]', a:flags,
          ...
          \ [0, a:fromlnum - s:maxoff]->max(), g:mojo_indent.searchpair_timeout)
endfunction
...
function mojo#GetIndent(lnum, ...)
  ...
  if getline(a:lnum - 1) =~ '\\$'
    ...
    return indent(a:lnum - 1) + get(g:, 'pyindent_continue', g:mojo_indent.continue)->eval()
  endif
  ...
  if g:mojo_indent.disable_parentheses_indenting == 1
    ...
    if parlnum > 0
      if parcol != col([parlnum, '$']) - 1
        return parcol
      elseif getline(a:lnum) =~ '^\s*[])}]' && !g:mojo_indent.closed_paren_align_last_line
    ...
    if parlnum > 0
      if a:0 > 0 && ExtraFunc(parlnum)
        " We may have found the opening brace of a bitbake Mojo task, e.g. 'mojo do_task {'
    ...
    if p > 0
      if a:0 > 0 && ExtraFunc(p)
        ...
        " Handle first non-empty line inside a bitbake Mojo task
        if p == plnum
          return shiftwidth()
        endif

        " Handle the user actually trying to close a bitbake Mojo task
        ...
        if p == plnum
          ...
          if pp > 0
            return indent(plnum)
              \ + get(g:, 'pyindent_nested_paren', g:mojo_indent.nested_paren)->eval()
          endif
          return indent(plnum)
            \ + get(g:, 'pyindent_open_paren', g:mojo_indent.open_paren)->eval()
        endif
        if plnumstart == p
          return indent(plnum)
        endif
